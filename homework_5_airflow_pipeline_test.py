...
def return_snowflake_conn():
    hook = SnowflakeHook(snowflake_conn_id='snowflake_default')  # Ensure connection name is correct
    conn = hook.get_conn()
    return conn.cursor()

# Task 1: Extract Data from Alpha Vantage API
@task
def extract(symbol="AAPL"):
    api_key = Variable.get("alpha_vantage_api_key")  # Fetch API key from Airflow Variables
    ...
    df['symbol'] = symbol  # ✅ Ensures only AAPL is stored
...
def load(records, target_table):
    ...
    try:
        ...
        con.execute(f"DELETE FROM {target_table} WHERE symbol = 'AAPL'")
